...
RenderableRoundCornerQuad::RenderableRoundCornerQuad(Interface* parent, float width, float height, bool border, float* bcolor, float* acolor, float cornersize, bool d, bool d3, float inOpacity)
	:Renderable(Programs::ShapeProgram, parent), bBorder(border), bDynamic(d), opacity(inOpacity)
{
	// Save Data
...
void RenderableRoundCornerQuad::Render()
{
	// Set up states
	...
	// Draw Face
	// glUniformMatrix4fv(1, 1, GL_FALSE, glm::value_ptr(parentInterface->project2DView*modelResize));	// MVP
...
void RenderableRoundCornerQuad::UpdateBuffer(float width, float height, float cornersize)
{
	// Clamp Border Size
...
void RenderableRoundCornerQuad::UpdateProperty(bool border, float* bcolor, float* acolor, float inOpacity)
{
	opacity = inOpacity;
	memcpy(borderColor, bcolor, sizeof(float) * 3);
	memcpy(areaColor, acolor, sizeof(float) * 3);
